...
def transform(feat_mat, initial_cond, Omega, K, trajectory_len, epsilon, threshold):
    """
    This function takes an input feature matrix with 6 tuning parameters
    for estimating features using a chaotic trajectory along the standard circle map.
    ...
    if not validate(feat_mat, initial_cond, Omega, K, trajectory_len, epsilon, threshold):
        return None
    ...
    trajectory = cs.compute_trajectory(initial_cond, Omega, K, trajectory_len)
...
def warmup():
    ...
    cs.warmup()
    out = transform(
        feat_mat, initial_cond=0.1, Omega=1/3, K=2.0, trajectory_len=100, epsilon=0.01, threshold=0.2
    )
